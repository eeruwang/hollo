...
export const relations = defineRelations(schema, (r) => ({
  ...
  webhooks: {
    owner: r.one.accountOwners({
      from: r.webhooks.accountOwnerId,
      to: r.accountOwners.id,
      optional: false,
    }),
  },
  filters: {
    owner: r.one.accountOwners({
      from: r.filters.accountOwnerId,
      to: r.accountOwners.id,
      optional: false,
    }),
    keywords: r.many.filterKeywords({
      from: r.filters.id,
      to: r.filterKeywords.filterId,
    }),
  },
  filterKeywords: {
    filter: r.one.filters({
      from: r.filterKeywords.filterId,
      to: r.filters.id,
      optional: false,
    }),
  },
